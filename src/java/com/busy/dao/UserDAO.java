...
public class UserDAO extends BasicConnection implements Serializable
{
    ...
    {
        if (column.equals(User.PROP_USER_ID) || column.equals(User.PROP_STATUS) || column.equals(User.PROP_RANK) || column.equals(User.PROP_BRAND_ID) || column.equals(User.PROP_USER_TYPE_ID) || column.equals(User.PROP_ADDRESS_ID) || column.equals(User.PROP_CONTACT_ID) || column.equals(User.PROP_USER_GROUP_ID))
        {
            return true;
        }
        else
    ...
    public static int addUser(String Username, String Password, String Email, String SecurityQuestion, String SecurityAnswer, Date RegisterDate, String ImageURL, Integer Status, Integer Rank, String WebUrl, Integer BrandId, Integer UserTypeId, Integer AddressId, Integer ContactId, Integer UserGroupId)
    {
        int id = 0;
        try
        {
            ...
            openConnection();
            prepareStatement("INSERT INTO user(Username,Password,Email,SecurityQuestion,SecurityAnswer,RegisterDate,ImageURL,Status,Rank,WebUrl,BrandId,UserTypeId,AddressId,ContactId,UserGroupId) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
            ...
            preparedStatement.setInt(11, BrandId);
    ...
    public static void updateUser(Integer UserId, String Username, String Password, String Email, String SecurityQuestion, String SecurityAnswer, Date RegisterDate, String ImageURL, Integer Status, Integer Rank, String WebUrl, Integer BrandId, Integer UserTypeId, Integer AddressId, Integer ContactId, Integer UserGroupId)
    {
        try
        {
            ...
            openConnection();
            prepareStatement("UPDATE user SET Username=?,Password=?,Email=?,SecurityQuestion=?,SecurityAnswer=?,RegisterDate=?,ImageURL=?,Status=?,Rank=?,WebUrl=?,BrandId=?,UserTypeId=?,AddressId=?,ContactId=?,UserGroupId=? WHERE UserId=?;");
            preparedStatement.setString(1, Username);
            preparedStatement.setString(2, Password);
            preparedStatement.setString(3, Email);
            preparedStatement.setString(4, SecurityQuestion);
