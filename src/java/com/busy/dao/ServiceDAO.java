...
import com.busy.entity.Service;
...
public class ServiceDAO extends BasicConnection implements Serializable
{
    ...
    {
        if (column.equals(Service.PROP_SERVICE_ID) || column.equals(Service.PROP_SERVICE_NAME) || column.equals(Service.PROP_DESCRIPTION) || column.equals(Service.PROP_STATUS) || column.equals(Service.PROP_SERVICE_CHARGE_ID) || column.equals(Service.PROP_SERVICE_TYPE_ID))
        {
            return column;
        }
        else
    ...
    {
        if (column.equals(Service.PROP_SERVICE_ID) || column.equals(Service.PROP_STATUS) || column.equals(Service.PROP_SERVICE_CHARGE_ID) || column.equals(Service.PROP_SERVICE_TYPE_ID))
        {
            return true;
        }
        else
    ...
    public static int addService(String ServiceName, String Description, Integer Status, Integer ServiceChargeId, Integer ServiceTypeId)
    {
        int id = 0;
        try
        {
            ...
            openConnection();
            prepareStatement("INSERT INTO service(ServiceName,Description,Status,ServiceChargeId,ServiceTypeId) VALUES (?,?,?,?,?);");
    ...
    public static void updateService(Integer ServiceId, String ServiceName, String Description, Integer Status, Integer ServiceChargeId, Integer ServiceTypeId)
    {
        try
        {
            ...
            openConnection();
            prepareStatement("UPDATE service SET ServiceName=?,Description=?,Status=?,ServiceChargeId=?,ServiceTypeId=? WHERE ServiceId=?;");
            preparedStatement.setString(1, ServiceName);
            preparedStatement.setString(2, Description);
            preparedStatement.setInt(3, Status);
            preparedStatement.setInt(4, ServiceChargeId);
