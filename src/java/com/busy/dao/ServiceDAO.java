    ...
    {    
        ...
        {            
            if(column.equals(Service.PROP_SERVICE_ID) || column.equals(Service.PROP_SERVICE_NAME) || column.equals(Service.PROP_DESCRIPTION) || column.equals(Service.PROP_SERVICE_CHARGE_ID) || column.equals(Service.PROP_SERVICE_TYPE_ID) )
            {
                return column;
            }
            else
        ...
        {
            if (column.equals(Service.PROP_SERVICE_ID) || column.equals(Service.PROP_SERVICE_CHARGE_ID) || column.equals(Service.PROP_SERVICE_TYPE_ID) )
            {
                return true;
            }        
            else
        ...
        {        
            return new Service(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5));
        }
        
        public static int addService(String ServiceName, String Description, Integer ServiceChargeId, Integer ServiceTypeId)
        {   
            int id = 0;
            try
            {
                ...
                openConnection();
                prepareStatement("INSERT INTO service(ServiceName,Description,ServiceChargeId,ServiceTypeId) VALUES (?,?,?,?);");                    
                preparedStatement.setString(1, ServiceName);
                preparedStatement.setString(2, Description);
                preparedStatement.setInt(3, ServiceChargeId);
        ...
        public static void updateService(Integer ServiceId,String ServiceName,String Description,Integer ServiceChargeId,Integer ServiceTypeId)
        {  
            try
            {   
                ...
                openConnection();                           
                prepareStatement("UPDATE service SET ServiceName=?,Description=?,ServiceChargeId=?,ServiceTypeId=? WHERE ServiceId=?;");                    
                preparedStatement.setString(1, ServiceName);
                preparedStatement.setString(2, Description);
                preparedStatement.setInt(3, ServiceChargeId);
                preparedStatement.setInt(4, ServiceTypeId);
