...
public class PageDaoImpl extends BasicConnection implements Serializable, PageDao
{
    ...
    public Page findWithInfo(Integer id)
    {
        ...
        try
        {
            ...
            getRecordById("meta_tag", page.getMetaTagId().toString());
    ...
    @Override
    public ArrayList<Page> findByColumns(Column... columns)
    {
        ArrayList<Page> pageList = new ArrayList<>();
        ...
        try
        {
            ...
            {
                c.setNumeric(Page.isColumnNumeric(c.getColumnName()));
            }

            getAllRecordsByColumns("page", columns);
            while (rs.next())
            {
                pageList.add(Page.process(rs));
            }
        ...
        {
            System.out.println("Page's method findByColumns(Column... columns) error: " + ex.getMessage());
        }
        finally
        {
            closeConnection();
