...
public class UserDaoImpl extends BasicConnection implements Serializable, UserDao
{
    ...
    public User findWithInfo(Integer id)
    {
        ...
        try
        {
            ...
            getRecordById("item_brand", user.getItemBrandId().toString());
            user.setItemBrand(ItemBrand.process(rs));

            getRecordById("user_type", user.getUserTypeId().toString());
            ...
            getRecordById("user_group", user.getUserGroupId().toString());
    ...
    @Override
    public ArrayList<User> findByColumns(Column... columns)
    {
        ArrayList<User> userList = new ArrayList<>();
        ...
        try
        {
            ...
            {
                c.setNumeric(User.isColumnNumeric(c.getColumnName()));
            }

            getAllRecordsByColumns("user", columns);
            while (rs.next())
            {
                userList.add(User.process(rs));
            }
        ...
        {
            System.out.println("User's method findByColumns(Column... columns) error: " + ex.getMessage());
        }
        finally
        {
            closeConnection();
