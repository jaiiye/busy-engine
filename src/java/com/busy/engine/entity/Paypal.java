...
public class Paypal extends AbstractEntity implements EntityItem<Integer>
{
    ...
    public void addJson(JsonObjectBuilder builder)
    {
        ...
        builder.add("paypalId", paypalId == null ? 0 : paypalId);

        builder.add("payPalUrl", payPalUrl == null ? "" : payPalUrl);

        builder.add("currencyCode", currencyCode == null ? "" : currencyCode);

        builder.add("apiUsername", apiUsername == null ? "" : apiUsername);

        builder.add("apiPassword", apiPassword == null ? "" : apiPassword);

        builder.add("apiSignature", apiSignature == null ? "" : apiSignature);

        builder.add("apiEndpoint", apiEndpoint == null ? "" : apiEndpoint);

        builder.add("activeProfile", activeProfile == null ? "" : activeProfile.toString());

        builder.add("returnUrl", returnUrl == null ? "" : returnUrl);

        builder.add("cancelUrl", cancelUrl == null ? "" : cancelUrl);

        builder.add("paymentType", paymentType == null ? "" : paymentType);

        builder.add("environment", environment == null ? "" : environment);
