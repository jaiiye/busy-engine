...
import com.busy.engine.service.ServiceChargeServiceImpl;
...
@WebServlet("/rest/ServiceCharge/*")
public class ServiceChargeHandler extends AbstractHandler
{
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        else
        {
            try
            {
                ...
                {
                    case "store":
                        generateStoreServiceResult(new ServiceChargeServiceImpl(request.getSession().getServletContext()).store(sessionUser.getUsername(), getIntegerValue(obj.get("serviceChargeId")), obj.getString("chargeName"), obj.getString("description"), obj.getString("rate"), obj.getString("units"), getOperatingDateFormat().parse(obj.getString("date")), getIntegerValue(obj.get("userServiceId"))), out);
                        break;
                    case "remove":
    ...
    {
        return "Handles the requests for ServiceCharge resource with the following format: rest/ServiceCharge/{Id:optional}";
    }
}
